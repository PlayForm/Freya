...
impl TestingHandler {
	...
	pub fn config(&mut self) -> &mut TestingConfig { &mut self.config }

	/// Provide some values to the app
	fn provide_vdom_contexts(&mut self) {
		self.vdom.insert_any_root_context(Box::new(self.platform_event_emitter.clone()));
	...
	pub async fn wait_for_update(&mut self) -> (bool, bool) {
		...
		timeout(self.config.vdom_timeout(), self.vdom.wait_for_work()).await.ok();
	...
	fn measure_text_group(&self, text_measurement:TextGroupMeasurement) {
		let sdom = self.utils.sdom();
		sdom.get().measure_paragraphs(text_measurement, SCALE_FACTOR);
	}

	/// Push an event to the events queue
	pub fn push_event(&mut self, event:PlatformEvent) { self.events_queue.push(event); }
	...
	pub fn cursor_icon(&self) -> CursorIcon { self.cursor_icon }

	/// Get the [SafeDOM]
	pub fn sdom(&self) -> &SafeDOM { self.utils.sdom() }
