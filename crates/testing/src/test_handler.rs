...
use crate::{
	config::TestingConfig, test_node::TestNode, test_utils::TestUtils,
	SCALE_FACTOR,
};
...
impl TestingHandler {
	...
	fn provide_vdom_contexts(&mut self) {
		self.vdom.insert_any_root_context(Box::new(
			self.platform_event_emitter.clone(),
		));
		self.vdom
				...
				match ev {
					EventMessage::RequestRerender => {
						if let Some(ticker) = ticker.as_mut() {
							...
							timeout(
								self.config.vdom_timeout(),
								self.vdom.wait_for_work(),
							)
							.await
							.ok();
						}
	...
	pub fn save_snapshot(&mut self, snapshot_path: impl Into<PathBuf>) {
		...
		// Save snapshot
		let mut snapshot_file = File::create(snapshot_path.into())
			.expect("Failed to create the snapshot file.");
