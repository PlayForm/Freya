...
impl TestUtils {
	/// Get the SafeDOM.
	pub fn sdom(&self) -> &SafeDOM { &self.sdom }
	...
	pub fn get_node_by_id(&self, node_id:NodeId) -> TestNode {
		...
		TestNode { node_id, utils, children_ids, height, state, node_type }
	}
	...
	pub fn get_node_matching_inside_id(
		...
		traverse_dom(rdom, node_id, |node| {
			if matcher(node) {
				...
				nodes.push(TestNode { node_id, utils, children_ids, height, state, node_type });
			}
		});
