...
impl TestNode {
	/// Quickly get a child of the Node by the given index, if the child is not
	/// found it will panic
	#[track_caller]
	...
	/// Get the Node text
	pub fn text(&self) -> Option<&str> { self.node_type.text() }
	...
	pub fn layout(&self) -> Option<LayoutNode> {
		self.utils().sdom().get().layout().get(self.node_id).cloned()
	}

	/// Get the Node layout Area
	pub fn area(&self) -> Option<Area> { self.layout().map(|l| l.area) }
	...
	pub fn utils(&self) -> &TestUtils { &self.utils }
	...
	pub fn dom_height(&self) -> u16 { self.height }
	...
	pub fn children_ids(&self) -> Vec<NodeId> { self.children_ids.clone() }

	/// Check if this element is text
	pub fn is_element(&self) -> bool { self.node_type.is_element() }

	/// Check if this element is text
	pub fn is_text(&self) -> bool { self.node_type.is_text() }

	/// Check if this element is a placeholder
	pub fn is_placeholder(&self) -> bool { self.node_type.is_placeholder() }
