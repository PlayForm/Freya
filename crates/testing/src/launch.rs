...
use crate::{
	config::TestingConfig, test_handler::TestingHandler, test_utils::TestUtils,
	SCALE_FACTOR,
};
...
pub fn launch_test_with_config(
	...
	let (platform_event_emitter, platform_event_receiver) =
		unbounded_channel::<EventMessage>();
	let (platform_sender, platform_receiver) =
		watch::channel(NativePlatformState {
			...
			information: PlatformInformation::new(
				config.size,
				false,
				false,
				false,
	...
	let mut handler = TestingHandler {
		...
		accessibility_manager: AccessibilityManager::new(ACCESSIBILITY_ROOT_ID)
			.wrap(),
