...
use freya_core::{
	accessibility::ACCESSIBILITY_ROOT_ID, platform_state::NavigationMode,
	prelude::EventMessage, types::AccessibilityId,
};
use freya_elements::events::{keyboard::Code, KeyboardEvent};
use freya_node_state::CustomAttributeValues;

use crate::{
	use_platform, AccessibilityIdCounter, NavigationMark, UsePlatform,
};
...
impl UseFocus {
	...
	pub fn attribute(&self) -> AttributeValue {
		AttributeValue::any_value(CustomAttributeValues::AccessibilityId(
			self.id,
		))
	}
...
pub fn use_focus() -> UseFocus {
	...
	let is_selected = use_memo(move || {
		*is_focused.read()
			&& *navigation_mode.read() == NavigationMode::Keyboard
	});
