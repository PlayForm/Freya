...
impl UseFocus {
	...
	pub fn focus(&mut self) {
		if !*self.is_focused.peek() {
			self.platform.send(EventMessage::FocusAccessibilityNode(self.id)).ok();
		}
	}
	...
	pub fn queue_focus(&mut self) {
		if !*self.is_focused.peek() {
			self.platform.send(EventMessage::QueueFocusAccessibilityNode(self.id)).ok();
		}
	}

	/// Get the node focus ID
	pub fn id(&self) -> AccessibilityId { self.id }
	...
	pub fn is_focused(&self) -> bool { *self.is_focused.read() }
	...
	pub fn prevent_navigation(&mut self) { self.navigation_mark.write().set_allowed(false); }
}
...
pub fn use_focus() -> UseFocus {
	...
	use_hook(move || {
		UseFocus { id, is_focused, is_selected, navigation_mode, navigation_mark, platform }
	})
}
