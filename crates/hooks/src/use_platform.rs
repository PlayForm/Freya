...
impl UsePlatform {
	...
	pub fn new() -> Self {
		UsePlatform {
			event_loop_proxy: Signal::new(try_consume_context::<
				EventLoopProxy<EventMessage>,
			>()),
			platform_emitter: Signal::new(try_consume_context::<
	...
	pub fn with_window(
		&self,
		cb: impl FnOnce(&Window) + 'static + Send + Sync,
	) {
	...
	pub fn toggle_minimize_window(&self) {
		self.with_window(|window| {
			window.set_minimized(
				window.is_minimized().map(|v| !v).unwrap_or_default(),
			);
		});
	}
	...
	pub fn new_ticker(&self) -> Ticker {
		Ticker { inner: self.ticker.peek().resubscribe() }
	}
