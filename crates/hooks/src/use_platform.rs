...
impl UsePlatform {
	...
	pub fn request_animation_frame(&self) { self.send(EventMessage::RequestRerender).ok(); }

	pub fn new_ticker(&self) -> Ticker { Ticker { inner:self.ticker.peek().resubscribe() } }

	/// Closes the whole app.
	pub fn exit(&self) { self.send(EventMessage::ExitApp).ok(); }
}

/// Get access to information and features of the platform.
pub fn use_platform() -> UsePlatform { use_hook(UsePlatform::new) }
...
impl Ticker {
	pub async fn tick(&mut self) { self.inner.recv().await.ok(); }
}
