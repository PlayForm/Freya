...
pub fn use_camera(camera_settings:CameraSettings) -> (AttributeValue, Signal<Option<NokhwaError>>) {
		...
		spawn(async move {
			...
			if let Ok(mut camera) = camera {
				...
				if let Some(resolution) = camera_settings.resolution {
					camera.set_resolution(resolution).unwrap_or_else(&mut handle_error);
				}
				...
				loop {
					...
					if let Ok(frame) = frame {
						...
						// Send the frame to the renderer via the image
						// reference
