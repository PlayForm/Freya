...
pub fn use_theme() -> Signal<Theme> { use_context::<Signal<Theme>>() }

/// Subscribe to [`Theme`] changes, default theme will be used if there is no
/// provided [`Theme`].
///
/// Primarily used by built-in components that have no control of whether they
/// will inherit a [`Theme`] or not.
...
/// 	let ButtonTheme { padding, width, background, .. } =
