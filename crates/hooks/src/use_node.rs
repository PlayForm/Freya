...
mod test {
	...
	pub async fn track_size() {
		...
		let mut utils = launch_test_with_config(
			use_node_app,
			TestingConfig { size:(500.0, 800.0).into(), ..TestingConfig::default() },
		);

		utils.wait_for_update().await;
		let root = utils.root().get(0);
		assert_eq!(root.get(0).get(0).text().unwrap().parse::<f32>(), Ok(500.0 * 0.5));

		utils.config().size = (300.0, 800.0).into();
		utils.wait_for_update().await;

		let root = utils.root().get(0);
		assert_eq!(root.get(0).get(0).text().unwrap().parse::<f32>(), Ok(300.0 * 0.5));
	}
}
