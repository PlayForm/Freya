...
/// # Safety
/// Skia `Paragraph` are neither Sync or Send, but in order to store them in the
/// Associated data of the Nodes in Torin (which will be used across threads
/// when making the attributes diffing), we must manually mark the Paragraph as
/// Send and Sync, this is fine because `Paragraph`s will only be accessed and
/// modified In the main thread when measuring the layout and painting.
