...
impl DisplayColor for Color {
	...
	fn fmt_hsl(&self, f:&mut fmt::Formatter) -> fmt::Result {
		...
		let s = if l == 1.0 || l == 0.0 { 0.0 } else { (hsv.v - l) / f32::min(l, 1.0 - l) };
...
fn parse_rgb(color:&str) -> Result<Color, ParseError> {
	...
	let r = colors.next().ok_or(ParseError)?.trim().parse::<u8>().map_err(|_| ParseError)?;
	let g = colors.next().ok_or(ParseError)?.trim().parse::<u8>().map_err(|_| ParseError)?;
	let b = colors.next().ok_or(ParseError)?.trim().parse::<u8>().map_err(|_| ParseError)?;
...
fn parse_hsl(color:&str) -> Result<Color, ParseError> {
	...
	let h = h_str.replacen("deg", "", 1).parse::<f32>().map_err(|_| ParseError)?;
	let mut s = s_str.replacen('%', "", 1).parse::<f32>().map_err(|_| ParseError)? / 100.0;
	let mut l = l_str.replacen('%', "", 1).parse::<f32>().map_err(|_| ParseError)? / 100.0;
	...
	if let Some(a_str) = a_str {
		...
		let a = a_str.trim().replace('%', "").parse::<f32>().map_err(|_| ParseError)? / 100.0;
