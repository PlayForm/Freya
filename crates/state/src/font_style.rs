...
use crate::{CustomAttributeValues, ExtSplit, Parse, ParseAttribute, TextOverflow};
...
impl FontStyleState {
	pub fn text_style(&self, default_font_family:&[String], scale_factor:f32) -> TextStyle {
		...
		text_style
			...
			.set_font_style(FontStyle::new(self.font_weight, self.font_width, self.font_slant))
	...
	fn default() -> Self {
		Self {
			...
			decoration:Decoration {
				thickness_multiplier:1.0, /* Defaults to 0.0, even though 0.0
		...
		match attr.attribute {
			AttributeName::Color => {
				if let Some(value) = attr.value.as_text() {
					// Make an exception for the "inherit" as in this case we
					// don't want to pass  a color at all but use the
					// inherited one.
					if value != "inherit" {
			...
			AttributeName::FontFamily => {
				if let Some(value) = attr.value.as_text() {
					...
					self.font_family =
						families.into_iter().map(|f| f.trim().to_string()).collect::<Vec<String>>();
				}
			},
