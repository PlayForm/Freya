...
// Custom measurer, useful to measure certain elements such as text with other
// libraries
...
impl DemoDOM {
	...
	pub fn add(&mut self, node_id:usize, parent:Option<usize>, children:Vec<usize>, node:Node) {
		...
		let parent_height = parent.map(|p| self.nodes.get(&p).unwrap().height).unwrap_or(0);

		// Assign the node a height just below its parent
		let height = parent_height + 1;

		self.nodes.insert(node_id, DemoNode { parent, children, height, node });
	}
...
impl DOMAdapter<usize> for DemoDOM {
	fn children_of(&mut self, node_id:&usize) -> Vec<usize> {
		self.nodes.get(node_id).map(|c| c.children.clone()).unwrap_or_default()
	}
	...
	fn height(&self, node_id:&usize) -> Option<u16> { self.nodes.get(node_id).map(|c| c.height) }
	...
	fn is_node_valid(&mut self, _node_id:&usize) -> bool {
		// We assume all the nodes in this Demo DOM are actually available for
		// measurement This could not be the case in certain implementations
		// of DOMs, for example Dioxus has a concept of Placeholders, which
		// are elements in the DOM but are not to be measured
		true
	}
...
fn main() {
	...
	demo_dom.set_node(
		1, // ID
		Node::from_size_and_direction(
			Size::Percentage(Length::new(80.0)), /* We change this from 50%
			                                      * to 80% */
			Size::Percentage(Length::new(80.0)), /* We change this from 50%
			                                      * to 80% */
			DirectionMode::Vertical,
	...
	// Make Torin calculate from what Node it is the most efficiente to start
	// measuring again
	layout.find_best_root(&mut demo_dom);

	// If Torin wasn't able to find a Root candidate, it will just use the ID we
	// pass as fist argument
	layout.measure(
		0,                                                              // Fallback Root ID
		Rect::new(Point2D::new(0.0, 0.0), Size2D::new(1000.0, 1000.0)), // Available Area
		&mut measurer,
