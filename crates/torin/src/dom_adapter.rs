...
impl LayoutNode {
	...
	pub fn visible_area(&self) -> Area { self.area.without_gaps(&self.margin) }
}
...
pub trait DOMAdapter<Key:NodeKey> {
	...
	/// Check whether the given Node is valid (isn't a placeholder, unconnected
	/// node..)
	...
	fn closest_common_parent(&self, node_a:&Key, node_b:&Key) -> Option<Key> {
		...
		let (node_a, node_b) = match height_a.cmp(&height_b) {
			std::cmp::Ordering::Less => {
				(*node_a, balance_heights(self, *node_b, *node_a).unwrap_or(*node_b))
			},
			std::cmp::Ordering::Equal => (*node_a, *node_b),
			std::cmp::Ordering::Greater => {
				(balance_heights(self, *node_a, *node_b).unwrap_or(*node_a), *node_b)
			},
		};
