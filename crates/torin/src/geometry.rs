use crate::prelude::{DirectionMode, Gaps, Node, Size};
...
pub trait AreaModel {
	...
	/// Adjust the available area with the node offsets (mainly used by
	/// scrollviews)
...
impl AreaModel for Area {
	fn without_gaps(self, gaps:&Gaps) -> Area {
		...
		Area::new(
			...
			Size2D::new(size.width - gaps.horizontal(), size.height - gaps.vertical()),
		)
	}
