...
pub enum PointerType {
	Mouse { trigger_button:Option<MouseButton> },
	Touch { finger_id:u64, phase:TouchPhase, force:Option<Force> },
}
...
impl PointerData {
	pub fn new(
		...
		Self { screen_coordinates, element_coordinates, pointer_type:point_type }
	}
}

impl PointerData {
	/// Get the mouse coordinates relative to the window bounds.
	pub fn get_screen_coordinates(&self) -> CursorPoint { self.screen_coordinates }

	/// Get the mouse coordinates relatives to the element bounds.
	pub fn get_element_coordinates(&self) -> CursorPoint { self.element_coordinates }

	/// Get the pointer type that triggered this event.
	pub fn get_pointer_type(&self) -> PointerType { self.pointer_type }
}

impl From<&PlatformEventData> for PointerData {
	fn from(val:&PlatformEventData) -> Self { val.downcast::<PointerData>().cloned().unwrap() }
}
