...
impl MouseData {
	pub fn new(
		...
		Self { screen_coordinates, element_coordinates, trigger_button }
	}
}
...
impl MouseData {
	...
	pub fn get_trigger_button(&self) -> Option<MouseButton> { self.trigger_button }
}

impl From<&PlatformEventData> for MouseData {
	fn from(val:&PlatformEventData) -> Self { val.downcast::<MouseData>().cloned().unwrap() }
}
