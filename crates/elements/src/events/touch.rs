...
impl TouchData {
	pub fn new(
		...
		Self { screen_coordinates, element_coordinates, finger_id, phase, force }
	}
	...
	pub fn get_finger_id(&self) -> u64 { self.finger_id }

	/// Get the touch phase of this event.
	pub fn get_touch_phase(&self) -> TouchPhase { self.phase }

	/// Get the touch force of this event.
	pub fn get_touch_force(&self) -> Option<Force> { self.force }
}

impl From<&PlatformEventData> for TouchData {
	fn from(val:&PlatformEventData) -> Self { val.downcast::<TouchData>().cloned().unwrap() }
}
