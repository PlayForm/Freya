...
impl DioxusState {
	...
	/// Attempt to convert an ElementId to a NodeId. This will return None if
	/// the ElementId is not in the RealDom.
...
impl<V:FromAnyValue + Send + Sync> WriteMutations for DioxusNativeCoreMutationWriter<'_, V> {
	fn register_template(&mut self, template:dioxus_core::prelude::Template) {
		...
		self.state.templates.insert(template.name.to_string(), template_root_ids);
	}
...
/// A trait that extends the `NodeImmutable` trait with methods that are useful
/// for dioxus.
pub trait NodeImmutableDioxusExt<V:FromAnyValue + Send + Sync>: NodeImmutable<V> {
	...
	/// Not all nodes are mounted to an element, only nodes with dynamic content
	/// that have been renderered will have an id.
