//! A custom element is a controlled element that renders to a shadow DOM. This
//! allows you to create elements that act like widgets without relying on a
//! specific framework.
//!
//! Each custom element is registered with a element name and namespace with
//! [`RealDom::register_custom_element`] or
//! [`RealDom::register_custom_element_with_factory`]. Once registered, they
//! will be created automatically when the element is added to the DOM.
...
impl<V:FromAnyValue + Send + Sync> Default for CustomElementRegistry<V> {
	fn default() -> Self { Self { builders:FxHashMap::default() } }
}
...
impl<V:FromAnyValue + Send + Sync> CustomElementRegistry<V> {
	...
	where
		...
		self.builders.insert(
			(F::NAME, F::NAMESPACE),
			CustomElementBuilder { create:|node| Box::new(F::create(node)) },
		);
	}
	...
	pub fn add_shadow_dom(&self, mut node:NodeMut<V>) {
		...
		if let Some((tag, ns)) = element_tag {
			if let Some(builder) = self.builders.get(&(tag.as_str(), ns.as_deref())) {
				...
				let boxed_custom_element =
					CustomElementManager { inner:Arc::new(RwLock::new(boxed_custom_element)) };
...
///
/// This is a simplified custom element trait for elements that can create
/// themselves. For more granular control, implement [`CustomElementFactory`]
/// and [`CustomElementUpdater`] instead.
pub trait CustomElement<V:FromAnyValue + Send + Sync = ()>: Send + Sync + 'static {
	...
	/// The node passed in is the light DOM node. The element should not modify
	/// the light DOM node, but it can get the [`NodeMut::real_dom_mut`] from
	/// the node to create new nodes.
	fn create(light_root:NodeMut<V>) -> Self;

	/// The root node of the custom element. These roots must be not change once
	/// the element is created.
	fn roots(&self) -> Vec<NodeId>;

	/// The slot to render children of the element into. The slot must be not
	/// change once the element is created.
	fn slot(&self) -> Option<NodeId> { None }
...
impl<W:CustomElement<V>, V:FromAnyValue + Send + Sync> CustomElementFactory<W, V> for W {
	...
	fn create(node:NodeMut<V>) -> Self { Self::create(node) }
}
...
impl<W:CustomElement<V>, V:FromAnyValue + Send + Sync> CustomElementUpdater<V> for W {
	...
	fn roots(&self) -> Vec<NodeId> { self.roots() }

	fn slot(&self) -> Option<NodeId> { self.slot() }
}
...
impl<V:FromAnyValue + Send + Sync> CustomElementManager<V> {
	...
	pub fn on_attributes_changed(&self, light_root:NodeMut<V>, attributes:&AttributeMask) {
		self.inner.write().unwrap().attributes_changed(light_root, attributes);
	}
}
