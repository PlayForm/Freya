...
impl<V: FromAnyValue + Send + Sync> CustomElementRegistry<V> {
	...
	pub fn add_shadow_dom(&self, mut node: NodeMut<V>) {
		...
		if let Some((tag, ns)) = element_tag {
			if let Some(builder) =
				self.builders.get(&(tag.as_str(), ns.as_deref()))
			{
				let boxed_custom_element =
					{ (builder.create)(node.reborrow()) };
...
pub trait CustomElementFactory<
	W: CustomElementUpdater<V>,
...
impl<W: CustomElement<V>, V: FromAnyValue + Send + Sync>
	CustomElementFactory<W, V> for W
{
...
impl<W: CustomElement<V>, V: FromAnyValue + Send + Sync> CustomElementUpdater<V>
	for W
{
	fn attributes_changed(
		&mut self,
