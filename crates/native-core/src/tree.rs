...
impl<'a> TreeMut for TreeMutView<'a> {
	fn remove(&mut self, id: NodeId) {
		fn recurse(tree: &mut TreeMutView<'_>, id: NodeId) {
			...
			if let Some(light_tree) = light_tree {
				let root_for_light_tree =
					(&mut tree.1).get(light_tree).unwrap();

				if let Some(shadow_tree) =
					&mut root_for_light_tree.child_subtree
				{
	...
	fn replace(&mut self, old_id: NodeId, new_id: NodeId) {
		{
			...
			if let Some(parent_id) = node_state.get(old_id).unwrap().parent {
				...
				let height =
					child_height((&self.1).get(parent_id).unwrap(), self);
	...
	fn insert_before(&mut self, old_id: NodeId, new_id: NodeId) {
		...
		let index =
			parent.children.iter().position(|child| *child == old_id).unwrap();
	...
	fn create_subtree(
		...
		{
			let shadow_tree =
				ShadowTree { shadow_roots: shadow_roots.clone(), slot };
			...
			if let Some(slot) = slot {
				let slot = node_data_mut.get(slot).expect(
					"tried to create shadow_tree with non-existent slot",
				);
		...
		if let Ok(node) = node_data_mut.get(id) {
			if let Some(shadow_tree) = node.child_subtree.take() {
				...
				if let Some(slot) = shadow_tree.slot {
					let slot = node_data_mut.get(slot).expect(
						"tried to remove shadow_tree with non-existent slot",
					);
...
fn shadow_tree() {
	...
	tree.create_subtree(
		parent_id,
		vec![shadow_parent_id],
		Some(shadow_child_id),
	);
