...
/// A shadow tree reference inside of a tree. This tree is isolated from the
/// main tree.
...
pub struct Node {
	...
	/// If this node is a slot in a shadow_tree, this is node whose
	/// child_subtree is that shadow_tree.
	slot_for_light_tree:Option<NodeId>,
	/// If this node is a root of a shadow_tree, this is the node whose
	/// child_subtree is that shadow_tree.
...
pub trait TreeRef {
	/// Get the id of the parent of the current node, if enter_shadow_dom is
	/// true and the current node is a shadow root, the node the shadow root is
	/// attached to will be returned
	#[inline]
	fn parent_id_advanced(&self, id:NodeId, enter_shadow_dom:bool) -> Option<NodeId> {
		// If this node is the root of a shadow_tree, return the node the
		// shadow_tree is attached
		...
		match (root_for_light_tree, enter_shadow_dom) {
			...
			_ => {
				...
				if enter_shadow_dom {
					// If this node is attached via a slot, return the slot as
					// the parent instead of the light tree parent
					parent_id
						.map(|id| self.shadow_tree(id).and_then(|tree| tree.slot).unwrap_or(id))
				} else {
					parent_id
				}
			},
	...
	fn children_ids_advanced(&self, id:NodeId, enter_shadow_dom:bool) -> Vec<NodeId> {
		...
		match (shadow_tree, slot_of_light_tree, enter_shadow_dom) {
			...
			// If this node is a slot, return the children of the node the slot
			// is attached to
...
pub trait TreeMut: TreeRef {
	/// Removes the node and its children from the tree but do not delete the
	/// entities.
...
impl<'a> TreeRef for TreeRefView<'a> {
	fn parent_id(&self, id:NodeId) -> Option<NodeId> { self.get(id).ok()?.parent }

	fn children_ids(&self, id:NodeId) -> Vec<NodeId> {
		self.get(id).map(|node| node.children.clone()).unwrap_or_default()
	}

	fn height(&self, id:NodeId) -> Option<u16> { Some(self.get(id).ok()?.height) }

	fn contains(&self, id:NodeId) -> bool { self.get(id).is_ok() }
...
impl<'a> TreeMut for TreeMutView<'a> {
	fn remove(&mut self, id:NodeId) {
		fn recurse(tree:&mut TreeMutView<'_>, id:NodeId) {
			...
			// If this node is a slot in a shadow_tree, remove it from the
			// shadow_tree.
	...
	fn insert_before(&mut self, old_id:NodeId, new_id:NodeId) {
		...
		let index = parent.children.iter().position(|child| *child == old_id).unwrap();
	...
	fn create_subtree(&mut self, id:NodeId, shadow_roots:Vec<NodeId>, slot:Option<NodeId>) {
		...
		{
			let shadow_tree = ShadowTree { shadow_roots:shadow_roots.clone(), slot };

			let light_root =
				node_data_mut.get(id).expect("tried to create shadow_tree with non-existent id");
		...
		// Now that we have created the shadow_tree, we need to update the
		// height of the shadow_tree roots
...
fn child_height(parent:&Node, tree:&impl TreeRef) -> u16 {
	match &parent.child_subtree {
		Some(shadow_tree) => {
			if let Some(slot) = shadow_tree.slot {
				tree.height(slot).expect("Attempted to read a slot that does not exist") + 1
			} else {
				panic!(
					"Attempted to read the height of a child of a node with a shadow tree, but \
					 the shadow tree does not have a slot. Every shadow tree attached to a node \
					 with children must have a slot."
				)
			}
		},
...
fn set_height(tree:&mut TreeMutView<'_>, node:NodeId, height:u16) {
	let (shadow_tree, light_tree, children) = {
		...
		(node.child_subtree.clone(), node.slot_for_light_tree, node.children.clone())
	};

	// If the children are actually part of a shadow_tree, there height is
	// determined by the height of the shadow_tree
	if let Some(shadow_tree) = shadow_tree {
		...
		// Otherwise, we just set the height of the children to be one more than
	...
	// If this nodes is a slot for a shadow_tree, we need to go to the super
	// tree and update the height of its children
	if let Some(light_tree) = light_tree {
...
impl<'a> TreeRef for TreeMutView<'a> {
	...
	fn children_ids(&self, id:NodeId) -> Vec<NodeId> {
		let node_data = &self.1;
		node_data.get(id).map(|node| node.children.clone()).unwrap_or_default()
	}
	...
	fn contains(&self, id:NodeId) -> bool { self.1.get(id).is_ok() }
...
fn shadow_tree() {
	...
	assert_eq!(
		tree.1.get(parent_id).unwrap().child_subtree.as_ref().unwrap().shadow_roots,
		&[shadow_parent_id]
	);
	assert_eq!(tree.1.get(shadow_child_id).unwrap().slot_for_light_tree, Some(parent_id));
