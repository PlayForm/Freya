...
impl<'a, V:FromAnyValue> NodeView<'a, V> {
	...
	pub fn new(id:NodeId, node:&'a NodeType<V>, view:&'a NodeMask, height:u16) -> Self {
		Self { inner:node, mask:view, id, height }
	}

	pub fn node_type(&self) -> &'a NodeType<V> { self.inner }
	...
	pub fn attributes<'b>(
		...
		match &self.inner {
			NodeType::Element(ElementNode { attributes, .. }) => {
				Some(
					attributes
						.iter()
						.filter(move |(attr, _)| self.mask.attritutes.contains(attr))
						.map(|(attr, val)| OwnedAttributeView { attribute:attr, value:val }),
				)
			},
			_ => None,
		}
	...
	pub fn text(&self) -> Option<&str> { self.mask.text.then_some(self.inner.text()).flatten() }
}
...
impl Default for AttributeMask {
	fn default() -> Self { AttributeMask::Some(FxHashSet::default()) }
}
...
impl NodeMask {
	...
	pub fn attributes(&self) -> &AttributeMask { &self.attritutes }

	/// Set the mask to view the tag
	pub fn set_tag(&mut self) { self.tag = true; }

	/// Get the mask for the tag
	pub fn tag(&self) -> bool { self.tag }

	/// Set the mask to view the text
	pub fn set_text(&mut self) { self.text = true; }

	/// Get the mask for the text
	pub fn text(&self) -> bool { self.text }

	/// Set the mask to view the listeners
	pub fn set_listeners(&mut self) { self.listeners = true; }

	/// Get the mask for the listeners
	pub fn listeners(&self) -> bool { self.listeners }
}
...
impl Default for AttributeMaskBuilder<'_> {
	fn default() -> Self { AttributeMaskBuilder::Some(&[]) }
}
