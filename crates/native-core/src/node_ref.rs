		...
		match &self.inner {
			NodeType::Element(ElementNode { attributes, .. }) => Some(
				attributes
					.iter()
					.filter(move |(attr, _)| {
						self.mask.attritutes.contains(attr)
					})
...
impl AttributeMask {
	...
	fn overlaps(&self, other: &Self) -> bool {
		match (self, other) {
			(AttributeMask::All, AttributeMask::Some(attrs)) => {
				!attrs.is_empty()
			},
			(AttributeMask::Some(attrs), AttributeMask::All) => {
				!attrs.is_empty()
			},
	...
	pub fn build(self) -> NodeMask {
		NodeMask {
			attritutes: match self.attritutes {
				...
				AttributeMaskBuilder::Some(attrs) => AttributeMask::Some(
					FxHashSet::from_iter(attrs.iter().copied()),
				),
			},
			tag: self.tag,
			text: self.text,
