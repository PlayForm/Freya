...
pub enum NodeType<V:FromAnyValue = ()> {
	...
	/// A placeholder node. This can be used as a cheaper placeholder for a node
...
impl<V:FromAnyValue> NodeType<V> {
	...
	pub fn is_element(&self) -> bool { matches!(self, Self::Element(..)) }
...
impl<V:FromAnyValue, S:Into<String>> From<S> for NodeType<V> {
	fn from(text:S) -> Self { Self::Text(text.into()) }
}

impl<V:FromAnyValue> From<ElementNode<V>> for NodeType<V> {
	fn from(element:ElementNode<V>) -> Self { Self::Element(element) }
}
...
/// The value of an attribute on a DOM node. This contains non-text values to
/// allow users to skip parsing attribute values in some cases.
#[derive(Clone)]
...
impl<V:FromAnyValue> From<V> for OwnedAttributeValue<V> {
	fn from(value:V) -> Self { Self::Custom(value) }
}
	...
	fn from(value:&dioxus_core::AttributeValue) -> Self {
		match value {
			...
			dioxus_core::AttributeValue::None => {
				panic!(
					"None attribute values result in removing the attribute, not converting it to \
					 a None value."
				)
			},
