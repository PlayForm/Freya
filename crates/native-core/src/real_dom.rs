//! A Dom that can sync with the VirtualDom mutations intended for use in lazy
//! renderers.
...
impl<V:FromAnyValue + Send + Sync> NodesDirty<V> {
	...
	fn mark_parent_added_or_removed(&mut self, node_id:NodeId) {
		...
		for pass in &*self.passes {
			// If any of the states in this node depend on the parent then mark
			// them as dirty
	...
	fn mark_child_changed(&mut self, node_id:NodeId) {
		let hm = self.passes_updated.entry(node_id).or_default();
		for pass in &*self.passes {
			// If any of the states in this node depend on the children then
			// mark them as dirty
...
/// A Dom that can sync with the VirtualDom mutations intended for use in lazy
/// renderers. The render state passes from parent to children and or
/// accumulates state from children to parents. To get started:
/// 1) Implement [crate::passes::State] for each part of your state that you
///    want to compute incrementally
...
/// 4) Call [RealDom::update_state] to update the state of incrementally
///    computed values on each node
///
/// # Custom attribute values
/// To allow custom values to be passed into attributes implement FromAnyValue
/// on a type that can represent your custom value and specify the V generic to
/// be that type. If you have many different custom values, it can be useful to
/// use a enum type to represent the variants.
...
impl<V:FromAnyValue + Send + Sync> RealDom<V> {
	/// Create a new RealDom with the given states that will be inserted and
	/// updated when needed
	pub fn new(tracked_states:impl Into<Box<[TypeErasedState<V>]>>) -> RealDom<V> {
		...
		for i in 1..=tracked_states.len() {
			...
			for state in before.iter_mut().chain(after.iter_mut()) {
				...
				// If this node depends on the other state as a parent, then the
				// other state should update its children of the current type
				// when it is invalidated
				if current.parent_dependancies_ids.contains(&state.this_type_id)
					&& !dependants.child.contains(&current_dependant)
				{
					dependants.child.push(current_dependant);
				}
				// If this node depends on the other state as a child, then the
				// other state should update its parent of the current type when
				// it is invalidated
				...
				// If this node depends on the other state as a sibling, then
				// the other state should update its siblings of the current
				// type when it is invalidated
			...
			// If the current state depends on itself, then it should update
			// itself when it is invalidated
		...
		passes_updated.insert(root_id, tracked_states.iter().map(|x| x.this_type_id).collect());
	...
	pub fn tree_ref(&self) -> TreeRefView { self.world.borrow().unwrap() }

	/// Get a mutable reference to the tree.
	pub fn tree_mut(&self) -> TreeMutView { self.world.borrow().unwrap() }

	/// Create a new node of the given type in the dom and return a mutable
	/// reference to it.
	pub fn create_node(&mut self, node:impl Into<NodeType<V>>) -> NodeMut<'_, V> {
		...
		self.dirty_nodes.mark_dirty(id, NodeMaskBuilder::ALL.build());
	...
	pub fn get_listeners(&self, event:&EventName) -> Vec<NodeRef<V>> {
		if let Some(nodes) = self.nodes_listening.get(event) {
			nodes.iter().map(|id| NodeRef { id:*id, dom:self }).collect()
		} else {
			Vec::new()
		}
	}
	...
	pub fn contains(&self, id:NodeId) -> bool { self.tree_ref().contains(id) }
	...
	/// Update the state of the dom, after appling some mutations. This will
	/// keep the nodes in the dom up to date with their VNode counterparts.
	pub fn update_state(
		&mut self,
		ctx:SendAnyMap,
		...
		for (node_id, passes) in passes {
			// remove any nodes that were created and then removed in the same
			// mutations from the dirty nodes list
	...
	/// calling the provided function on each node only when the parent function
	/// returns `true`. This is useful to not traverse through text nodes for
	/// instance.
	...
	/// Traverses the dom in a depth first manner, calling the provided function
	/// on each node.
	...
	/// Returns a reference to the underlying world. Any changes made to the
	/// world will not update the reactive system.
	pub fn raw_world(&self) -> &World { &self.world }

	/// Returns a mutable reference to the underlying world. Any changes made to
	/// the world will not update the reactive system.
...
impl<'a, V:Component + Send + Sync> ViewEntry<'a, V> {
	fn new(view:View<'a, V>, id:NodeId) -> Self { Self { view, id } }
}

impl<'a, V:Component + Send + Sync> Deref for ViewEntry<'a, V> {
	type Target = V;

	fn deref(&self) -> &Self::Target { &self.view[self.id] }
}
...
impl<'a, V:Component<Tracking = Untracked> + Send + Sync> ViewEntryMut<'a, V> {
	fn new(view:ViewMut<'a, V, Untracked>, id:NodeId) -> Self { Self { view, id } }
}

impl<'a, V:Component<Tracking = Untracked> + Send + Sync> Deref for ViewEntryMut<'a, V> {
	type Target = V;

	fn deref(&self) -> &Self::Target { self.view.get(self.id).unwrap() }
}

impl<'a, V:Component<Tracking = Untracked> + Send + Sync> DerefMut for ViewEntryMut<'a, V> {
	fn deref_mut(&mut self) -> &mut Self::Target { (&mut self.view).get(self.id).unwrap() }
}
...
pub trait NodeImmutable<V:FromAnyValue + Send + Sync = ()>: Sized {
	...
	#[inline]
	fn node_type(&self) -> ViewEntry<NodeType<V>> { self.get().unwrap() }
	...
	fn get<'a, T:Component + Sync + Send>(&'a self) -> Option<ViewEntry<'a, T>> {
		...
		view.contains(self.id()).then(|| ViewEntry::new(view, self.id()))
	}

	/// Get the ids of the children of the current node, if enter_shadow_dom is
	/// true and the current node is a shadow slot, the ids of the nodes under
	/// the node the shadow slot is attached to will be returned
	#[inline]
	fn children_ids_advanced(&self, id:NodeId, enter_shadow_dom:bool) -> Vec<NodeId> {
		self.real_dom().tree_ref().children_ids_advanced(id, enter_shadow_dom)
	}

	/// Get the ids of the children of the current node
	#[inline]
	fn child_ids(&self) -> Vec<NodeId> { self.real_dom().tree_ref().children_ids(self.id()) }
	...
	fn children(&self) -> Vec<NodeRef<V>> {
		self.child_ids()
			.iter()
			.map(|id| NodeRef { id:*id, dom:self.real_dom() })
			.collect()
	}

	/// Get the id of the parent of the current node
	#[inline]
	fn parent_id(&self) -> Option<NodeId> { self.real_dom().tree_ref().parent_id(self.id()) }

	/// Get the parent of the current node
	#[inline]
	fn parent(&self) -> Option<NodeRef<V>> {
		self.parent_id().map(|id| NodeRef { id, dom:self.real_dom() })
	}

	/// Get the height of the current node in the tree (the number of nodes
	/// between the current node and the root)
	#[inline]
	fn height(&self) -> u16 { self.real_dom().tree_ref().height(self.id()).unwrap() }
}
...
impl<'a, V:FromAnyValue + Send + Sync> NodeImmutable<V> for NodeRef<'a, V> {
	#[inline(always)]
	fn real_dom(&self) -> &RealDom<V> { self.dom }

	#[inline(always)]
	fn id(&self) -> NodeId { self.id }
}

/// A mutable refrence to a node in the RealDom that tracks what States need to
/// be updated
...
impl<'a, V:FromAnyValue + Send + Sync> NodeMut<'a, V> {
	...
	pub fn new(id:NodeId, dom:&'a mut RealDom<V>) -> Self { Self { id, dom } }
}
...
impl<'a, V:FromAnyValue + Send + Sync> NodeMut<'a, V> {
	...
	#[inline(always)]
	pub fn real_dom_mut(&mut self) -> &mut RealDom<V> { self.dom }
	...
	pub fn get_mut<T:Component<Tracking = Untracked> + Sync + Send>(
		...
		view_mut.contains(self.id).then_some(ViewEntryMut::new(view_mut, self.id))
	}

	/// Insert a custom component into this node
	///
	/// Note: Components that implement State and are added when the RealDom is
	/// created will automatically be created
	#[inline]
	...
	pub fn remove(&mut self) {
		let id = self.id();
		{
			let RealDom { world, nodes_listening, .. } = &mut self.dom;
		...
		if let Some(parent_id) = parent_id {
			self.real_dom_mut().dirty_nodes.mark_child_changed(parent_id);
		}
	...
	pub fn add_event_listener(&mut self, event:EventName) {
		let id = self.id();
		let RealDom { world, dirty_nodes, nodes_listening, .. } = &mut self.dom;
		let mut view:ViewMut<NodeType<V>> = world.borrow().unwrap();
	...
	pub fn node_type_mut(&mut self) -> NodeTypeMut<'_, V> {
		let id = self.id();
		let RealDom { world, dirty_nodes, .. } = &mut self.dom;
		let view:ViewMut<NodeType<V>> = world.borrow().unwrap();
		let node_type = ViewEntryMut::new(view, id);
		match &*node_type {
			NodeType::Element(_) => {
				NodeTypeMut::Element(ElementNodeMut { id, element:node_type, dirty_nodes })
			},
			NodeType::Text(_) => NodeTypeMut::Text(TextNodeMut { id, text:node_type, dirty_nodes }),
	...
	pub fn set_type(&mut self, new:NodeType<V>) {
		...
		self.dom.dirty_nodes.mark_dirty(self.id, NodeMaskBuilder::ALL.build())
	}

	/// Clone a node and it's children and returns the id of the new node.
	/// This is more effecient than creating the node from scratch because it
	/// can pre-allocate the memory required.
	#[inline]
...
impl<V:FromAnyValue + Send + Sync> TextNodeMut<'_, V> {
	...
	pub fn text_mut(&mut self) -> &mut String {
		self.dirty_nodes.mark_dirty(self.id, NodeMaskBuilder::new().with_text().build());
		match &mut *self.text {
			NodeType::Text(text) => text,
			_ => unreachable!(),
		}
...
impl<V:FromAnyValue + Send + Sync> ElementNodeMut<'_, V> {
	...
	pub fn set_attribute(
		...
		self.dirty_nodes.mark_dirty(
			self.id,
			NodeMaskBuilder::new().with_attrs(AttributeMaskBuilder::Some(&[name])).build(),
		);
...
// Create a workload from all of the passes. This orders the passes so that each
// pass will only run at most once.
fn construct_workload<V:FromAnyValue + Send + Sync>(passes:&mut [TypeErasedState<V>]) -> Workload {
	...
	for i in 0..unresloved_workloads.len() {
		...
		for ty_id in all_dependancies {
			let &(dependancy_id, ..) = unresloved_workloads
				.iter()
				.find(|(_, pass, _)| pass.this_type_id == ty_id)
				.unwrap();
			let (_, _, workload) = &mut unresloved_workloads[i];
			*workload =
				workload.take().map(|workload| workload.after_all(dependancy_id.to_string()));
		}
	}
