...
use shipyard::{
	error::GetStorage, track::Untracked, Component, Get, IntoBorrow,
	ScheduledWorkload, SystemModificator, Unique, View, ViewMut, Workload,
	World,
};
...
impl<V: FromAnyValue + Send + Sync> NodesDirty<V> {
	...
	fn mark_dirty(&mut self, node_id: NodeId, mask: NodeMask) {
		self.passes_updated.entry(node_id).or_default().extend(
			self.passes.iter().filter_map(|x| {
				x.mask.overlaps(&mask).then_some(x.this_type_id)
			}),
		);
...
impl<V: FromAnyValue + Send + Sync> RealDom<V> {
	...
	pub fn update_state(
		...
		let dirty_nodes = DirtyNodeStates::with_passes(
			self.dirty_nodes.passes.iter().map(|p| p.this_type_id),
		);
	...
	pub fn traverse_depth_first_advanced(
		&self,
		mut f: impl FnMut(NodeRef<V>) -> bool,
	) {
...
impl<'a, V: Component<Tracking = Untracked> + Send + Sync> Deref
	for ViewEntryMut<'a, V>
{
	type Target = V;
...
impl<'a, V: Component<Tracking = Untracked> + Send + Sync> DerefMut
	for ViewEntryMut<'a, V>
{
...
pub trait NodeImmutable<V: FromAnyValue + Send + Sync = ()>: Sized {
	...
	#[inline]
	fn get<'a, T: Component + Sync + Send>(
		&'a self,
	) -> Option<ViewEntry<'a, T>> {
...
impl<'a, V: FromAnyValue + Send + Sync> NodeMut<'a, V> {
	...
	pub fn set_type(&mut self, new: NodeType<V>) {
		{
			let mut view: ViewMut<NodeType<V>> =
				self.dom.borrow_node_type_mut().unwrap();
...
fn construct_workload<V: FromAnyValue + Send + Sync>(
	...
	for i in 0..unresloved_workloads.len() {
		...
		let all_dependancies: Vec<_> =
			pass.combined_dependancy_type_ids().collect();
