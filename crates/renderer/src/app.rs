...
impl Application {
	...
	pub fn new(
		...
		app.plugins
			.send(PluginEvent::WindowCreated(window), PluginHandle::new(&app.proxy));
	...
	pub fn provide_vdom_contexts<State:'static>(&mut self, app_state:Option<State>) {
		...
		self.vdom.insert_any_root_context(Box::new(self.proxy.clone()));
		self.vdom.insert_any_root_context(Box::new(self.platform_receiver.clone()));
		self.vdom
	...
	pub fn init_doms<State:'static>(&mut self, scale_factor:f32, app_state:Option<State>) {
		self.plugins
			.send(PluginEvent::StartedUpdatingDOM, PluginHandle::new(&self.proxy));

		self.provide_vdom_contexts(app_state);

		self.sdom.get_mut().init_dom(&mut self.vdom, scale_factor);
		self.plugins
			.send(PluginEvent::FinishedUpdatingDOM, PluginHandle::new(&self.proxy));
	}

	/// Update the RealDOM, layout and others with the latest changes from the
	/// VirtualDOM
	pub fn render_mutations(&mut self, scale_factor:f32) -> (bool, bool) {
		...
		let (repaint, relayout) =
			self.sdom.get_mut().render_mutations(&mut self.vdom, scale_factor);
	...
	pub fn render(&mut self, hovered_node:&HoveredNode, canvas:&Canvas, window:&Window) {
		...
		self.start_render(hovered_node, canvas, window.inner_size(), window.scale_factor() as f32);

		self.accessibility.render_accessibility(window.title().as_str());
	...
	pub fn measure_text_group(&self, text_measurement:TextGroupMeasurement, scale_factor:f64) {
		self.sdom.get().measure_paragraphs(text_measurement, scale_factor);
	}

	pub fn focus_node(&mut self, node_id:AccessibilityId, window:&Window) {
		self.accessibility.focus_node(node_id, &self.platform_sender, window)
	}
	...
	pub fn focus_next_node(&mut self, direction:AccessibilityFocusDirection, window:&Window) {
		self.accessibility.focus_next_node(direction, &self.platform_sender, window)
	}

	/// Notify components subscribed to event loop ticks.
	pub fn event_loop_tick(&self) { self.ticker_sender.send(()).ok(); }
	...
	pub fn process_layout(&mut self, inner_size:PhysicalSize<u32>, scale_factor:f64) {
...

		{
			...
			self.plugins
				.send(PluginEvent::StartedLayout(&fdom.layout()), PluginHandle::new(&self.proxy));

			process_layout(
				&fdom,
				Area::from_size(Size2D::from((inner_size.width as f32, inner_size.height as f32))),
			...
			self.plugins
				.send(PluginEvent::FinishedLayout(&fdom.layout()), PluginHandle::new(&self.proxy));
		}
	...
	pub fn start_render(
		...
		process_render(&fdom, |fdom, node_id, layout_node, layout| {
			let render_wireframe = if let Some(hovered_node) = &hovered_node {
				hovered_node.lock().unwrap().map(|id| id == *node_id).unwrap_or_default()
			} else {
				false
			};
