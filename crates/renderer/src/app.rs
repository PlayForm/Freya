...
use crate::{
	accessibility::AccessKitManager, devtools::Devtools,
	winit_waker::winit_waker, EmbeddedFonts, HoveredNode,
};
...
impl Application {
	...
	pub fn new(
		...
		let (platform_sender, platform_receiver) =
			watch::channel(NativePlatformState {
				focused_id: ACCESSIBILITY_ROOT_ID,
				preferred_theme: window
					.theme()
	...
	pub fn provide_vdom_contexts<State: 'static>(
		&mut self,
	...
	pub fn poll_vdom(&mut self, window: &Window) {
		...
		let (must_repaint, must_relayout) =
			self.render_mutations(window.scale_factor() as f32);
