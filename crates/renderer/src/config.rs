...
impl LaunchConfig<'_, ()> {
	pub fn load_icon(icon: &[u8]) -> Icon {
		...
		let image =
			reader.decode().expect("Failed to open icon path").into_rgba8();
...
impl<'a, T: Clone> LaunchConfig<'a, T> {
	...
	pub fn with_background(mut self, background: &str) -> Self {
		self.window_config.background =
			Color::parse(background).unwrap_or(Color::WHITE);
		self
	}
	...
	pub fn with_window_attributes(
		mut self,
		window_attributes_hook: impl Fn(WindowAttributes) -> WindowAttributes
			+ 'static,
	) -> Self {
		self.window_config.window_attributes_hook =
			Some(Box::new(window_attributes_hook));
		self
	}
}
