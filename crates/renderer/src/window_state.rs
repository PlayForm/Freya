...
impl<'a, State:Clone + 'a> WindowState<'a, State> {
	...
	pub fn has_been_created(&self) -> bool { matches!(self, Self::Created(..)) }
	...
	pub fn create(
		...
		let Self::NotCreated(NotCreatedState { sdom, vdom, devtools, mut config }) =
		...
		load_with(|s| gl_config.display().get_proc_address(CString::new(s).unwrap().as_c_str()));
		...
		let mut surface =
			create_surface(&mut window, fb_info, &mut gr_context, num_samples, stencil_size);
