...
use glutin::{
	...
	surface::{
		GlSurface, Surface as GlutinSurface, SurfaceAttributesBuilder,
...
use crate::{
	app::Application, config::WindowConfig, devtools::Devtools, LaunchConfig,
};
...
impl<'a, State: Clone + 'a> WindowState<'a, State> {
	...
	pub fn create(
		...
		let mut window_attributes = Window::default_attributes()
			...
			.with_inner_size(LogicalSize::<f64>::from(
				config.window_config.size,
			));
		...
		if let Some(min_size) = config.window_config.min_size {
			window_attributes = window_attributes
				.with_min_inner_size(LogicalSize::<f64>::from(min_size));
		}
		if let Some(max_size) = config.window_config.max_size {
			window_attributes = window_attributes
				.with_max_inner_size(LogicalSize::<f64>::from(max_size));
		}

		if let Some(with_window_attributes) =
			&config.window_config.window_attributes_hook
		{
		...
		let mut window =
			window.expect("Could not create window with OpenGL context");
		...
		let gl_context = not_current_gl_context
			...
			.expect(
			"Could not make GL context current when setting up skia renderer",
		);
		...
		surface.canvas().scale((scale_factor as f32, scale_factor as f32));
