...
use freya_engine::prelude::{
	Color, FontStyle, Paint, PaintStyle, ParagraphBuilder, ParagraphStyle,
	Rect, Slant, TextShadow, TextStyle, Weight, Width,
};
...
impl FreyaPlugin for PerformanceOverlayPlugin {
	fn on_event(&mut self, event: &PluginEvent, _handle: PluginHandle) {
		match event {
			PluginEvent::StartedLayout(_) => {
				self.started_layout = Some(Instant::now())
			},
			PluginEvent::FinishedLayout(_) => {
				self.finished_layout =
					Some(self.started_layout.unwrap().elapsed())
			},
			PluginEvent::StartedUpdatingDOM => {
				self.started_dom_updates = Some(Instant::now())
			},
			PluginEvent::FinishedUpdatingDOM => {
				self.finished_dom_updates =
					Some(self.started_dom_updates.unwrap().elapsed())
			},
			PluginEvent::BeforeRender { .. } => {
				self.started_render = Some(Instant::now())
			},
			PluginEvent::AfterRender { canvas, font_collection, freya_dom } => {
				...
				self.frames.retain(|frame| {
					now.duration_since(*frame).as_millis() < 1000
				});
				...
				self.max_fps = self.max_fps.max(
					self.fps_historic.iter().max().copied().unwrap_or_default(),
				);
				let start_x = 5.0;
...
fn add_text(
	...
	let font_style =
		FontStyle::new(Weight::BOLD, Width::EXPANDED, Slant::Upright);
