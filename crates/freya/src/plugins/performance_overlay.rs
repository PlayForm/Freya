	...
	fn on_event(&mut self, event:&PluginEvent, _handle:PluginHandle) {
		match event {
			...
			PluginEvent::AfterRender { canvas, font_collection, freya_dom } => {
				...
				self.frames.retain(|frame| now.duration_since(*frame).as_millis() < 1000);
				...
				// FPS
				add_text(&mut paragraph_builder, format!("{} \n", self.frames.len()), 30.0);
				...
				self.max_fps =
					self.max_fps.max(self.fps_historic.iter().max().copied().unwrap_or_default());
				let start_x = 5.0;
...
fn add_text(paragraph_builder:&mut ParagraphBuilder, text:String, font_size:f32) {
	...
	text_style.add_shadow(TextShadow::new(Color::from_rgb(65, 65, 65), (0.0, 1.0), 1.0));
