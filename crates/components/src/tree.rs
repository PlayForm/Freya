...
impl ItemPath for PathBuf {
	fn item_starts_with(&self, other:&Self) -> bool { self.starts_with(other) }
}
...
pub enum TreeItem<I, V> {
	Expandable { id:I, value:V, state:ExpandableItemState<I, V> },
	Standalone { id:I, value:V },
}
	...
	V: Clone + PartialEq,
{
	...
	/// Update the state of the given Expandable Item, e.g to open with more
	/// items or to simply close.
	...
	pub fn flat(&self, depth:usize, root_id:&I) -> Vec<FlatItem<I>> {
		...
		if let TreeItem::Expandable { state: ExpandableItemState::Open(items), .. } = self {
			for item in items {
	...
	fn into_flat(self, depth:usize, root_id:I) -> FlatItem<I> {
		match self {
			TreeItem::Standalone { id, .. } => {
				FlatItem { id, is_standalone:true, is_open:false, depth, root_id }
			},
			TreeItem::Expandable { id, state, .. } => {
				FlatItem {
					id,
					is_standalone:false,
...
mod test {
	...
	fn tree() {
		...
		let mut tree = TreeItem::Expandable {
			...
			state:ExpandableItemState::Open(vec![
				TreeItem::Expandable {
					...
					value:(),
					state:ExpandableItemState::Open(vec![
						TreeItem::Standalone { id:PathBuf::from_str("/1/1").unwrap(), value:() },
						TreeItem::Standalone { id:PathBuf::from_str("/1/2").unwrap(), value:() },
					]),
				},
				TreeItem::Expandable {
				...
				TreeItem::Standalone { id:PathBuf::from_str("/3").unwrap(), value:() },
			]),
		};

		tree.set_state(&PathBuf::from_str("/1").unwrap(), &ExpandableItemState::Closed);
		tree.set_state(
		...
		tree.set_state(&PathBuf::from_str("/2/1").unwrap(), &ExpandableItemState::Closed);
		tree.set_state(&PathBuf::from_str("/3").unwrap(), &ExpandableItemState::Closed);
