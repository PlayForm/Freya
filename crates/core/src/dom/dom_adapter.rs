...
impl<'a> DioxusDOMAdapter<'a> {
	pub fn new(rdom:&'a DioxusDOM, scale_factor:f32) -> Self {
		Self { rdom, scale_factor, cache:FxHashMap::default() }
	}
}
...
impl DOMAdapter<NodeId> for DioxusDOMAdapter<'_> {
	fn get_node(&self, node_id:&NodeId) -> Option<Node> {
		...
		let contains_text = node.node_type().tag().map(|t| t.contains_text()).unwrap_or_default();
	...
	fn height(&self, node_id:&NodeId) -> Option<u16> { self.rdom.tree_ref().height(*node_id) }
	...
	fn root_id(&self) -> NodeId { self.rdom.root_id() }
}

/// Check is the given Node is valid or not, this means not being a placeholder
/// or an unconnected Node.
fn is_node_valid(rdom:&DioxusDOM, cache:&mut FxHashMap<NodeId, bool>, node_id:&NodeId) -> bool {
	...
	let is_valid = 'validation: {
		if let Some(node) = node {
			...
			// This walkes up to the ancestor that has a height of 0 and checks
			// if it has the same ID as the root Node If it has the same ID,
