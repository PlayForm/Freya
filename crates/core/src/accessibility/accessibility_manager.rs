...
use accesskit::{
	Action, DefaultActionVerb, Node, NodeBuilder, Rect, Role, Tree, TreeUpdate,
};
...
impl AccessibilityManager {
	pub fn new(focused_id: AccessibilityId) -> Self {
		Self { focused_id, nodes: Vec::default() }
	}
	...
	pub fn set_focus_on_next_node(
		...
		self.focused_id =
			target_node.map(|(id, _)| *id).unwrap_or(ACCESSIBILITY_ROOT_ID);
