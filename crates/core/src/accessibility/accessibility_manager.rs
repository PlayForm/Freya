...
impl AccessibilityManager {
	pub fn new(focused_id:AccessibilityId) -> Self { Self { focused_id, nodes:Vec::default() } }

	/// Wrap it in a `Arc<Mutex<T>>`.
	pub fn wrap(self) -> SharedAccessibilityManager { Arc::new(Mutex::new(self)) }

	/// Clear the Accessibility Nodes.
	pub fn clear(&mut self) { self.nodes.clear(); }

	pub fn push_node(&mut self, id:AccessibilityId, node:Node) { self.nodes.push((id, node)) }
	...
	pub fn set_focus_with_update(&mut self, new_focus_id:AccessibilityId) -> Option<TreeUpdate> {
		...
		if node_focused_exists {
			Some(TreeUpdate { nodes:Vec::new(), tree:None, focus:self.focused_id })
		} else {
			None
		}
	}
	...
	pub fn build_root(&mut self, root_name:&str) -> Node {
		...
		builder
			.set_children(self.nodes.iter().map(|(id, _)| *id).collect::<Vec<AccessibilityId>>());
	...
	pub fn process(&mut self, root_id:AccessibilityId, root_name:&str) -> TreeUpdate {
		...
		let focus = self
			.nodes
			.iter()
			.find_map(|node| if node.0 == self.focused_id { Some(node.0) } else { None })
	...
	pub fn set_focus_on_next_node(&mut self, direction:AccessibilityFocusDirection) -> TreeUpdate {
		...
		let target_node = if direction == AccessibilityFocusDirection::Forward {
			...
			if let Some(node_index) = node_index {
				if node_index == 0 { self.nodes.last() } else { self.nodes.get(node_index - 1) }
			} else {
				self.nodes.last()
			}
		};

		self.focused_id = target_node.map(|(id, _)| *id).unwrap_or(ACCESSIBILITY_ROOT_ID);

		TreeUpdate { nodes:Vec::new(), tree:None, focus:self.focused_id }
	}
}
