...
pub fn process_events(
	...
	let potential_events =
		measure_potential_event_listeners(events, dom, scale_factor);
	...
	let colateral_global_events =
		measure_colateral_global_events(&to_emit_dom_collateral_events);
				...
				'events: for event in events.iter() {
					if let PlatformEvent::Keyboard { name, .. } = event {
						...
						let data = match event {
							PlatformEvent::Mouse { name, cursor, .. } => {
								Some((name, cursor))
							},
							PlatformEvent::Wheel { name, cursor, .. } => {
								Some((name, cursor))
							},
							PlatformEvent::Touch { name, location, .. } => {
								Some((name, location))
							},
							PlatformEvent::File { name, cursor, .. } => {
								Some((name, cursor))
							},
							_ => None,
						};
						if let Some((name, cursor)) = data {
							...
							let cursor_is_inside = element_utils
							...
							if cursor_is_inside {
								...
								for node_id in &node_viewports.viewports {
									...
									let Some(element_utils) = node_type
										.tag()
										.and_then(|tag| tag.utils())
									else {
										continue;
									};
									let layout_node =
										layout.get(*node_id).unwrap();
...
fn measure_dom_events(
	...
	for (event_name, event_nodes) in potential_events {
		...
		'event: for collateral_event in collateral_events {
			...
			for PotentialEvent { node_id, event, layer } in
				event_nodes.iter().rev()
			{
				...
				let StyleState { background, .. } =
					&*node.get::<StyleState>().unwrap();
