...
pub fn process_events(
	...
	// 2. Get potential events that could be emitted based on the elements layout
	//    and viewports
	let potential_events = measure_potential_event_listeners(events, dom, scale_factor);

	// 3. Get what events can be actually emitted based on what elements are
	//    listening
	let dom_events = measure_dom_events(potential_events, dom, scale_factor);

	// 4. Filter the dom events and get potential collateral events, e.g. mouseover
	//    -> mouseenter
					...
					if let PlatformEvent::Keyboard { name, .. } = event {
						...
						if let Some((name, cursor)) = data {
							...
							if cursor_is_inside {
								...
								// Make sure the cursor is inside all the
								// applicable viewports from the element
...
fn measure_dom_events(
	...
	for (event_name, event_nodes) in potential_events {
		...
		'event: for collateral_event in collateral_events {
			...
			for PotentialEvent { node_id, event, layer } in event_nodes.iter().rev() {
					...
					&& !event.get_name().does_go_through_solid()
				{
					...
					// This only matters for events that bubble up (e.g. cursor
					// movement events)
		...
		for potential_event in valid_events {
			...
			if let Some(layout_node) = layout_node {
				let event =
					DomEvent::new(potential_event, Some(layout_node.visible_area()), scale_factor);
	...
	for global_event in global_events {
		...
		for listener in listeners {
			let event = DomEvent::new(
				PotentialEvent { node_id:listener.id(), layer:None, event:global_event.clone() },
				None,
				scale_factor,
			);
