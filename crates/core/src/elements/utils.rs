...
impl ElementUtilsResolver for TagName {
	fn utils(&self) -> Option<ElementWithUtils> {
		match self {
			...
			TagName::Paragraph => {
				Some(ElementWithUtils::Paragraph(ParagraphElement))
			},
...
impl ElementUtils for ElementWithUtils {
	fn clip(
		...
		match self {
			Self::Rect(el) => {
				el.clip(layout_node, node_ref, canvas, scale_factor)
			},
			Self::Svg(el) => {
				el.clip(layout_node, node_ref, canvas, scale_factor)
			},
	...
	fn is_point_inside_area(
		...
		match self {
			Self::Rect(el) => el.is_point_inside_area(
				point,
				node_ref,
				layout_node,
				scale_factor,
			),
			Self::Svg(el) => el.is_point_inside_area(
				point,
				node_ref,
				layout_node,
				scale_factor,
			),
			Self::Paragraph(el) => el.is_point_inside_area(
				point,
				node_ref,
				layout_node,
				scale_factor,
			),
			Self::Image(el) => el.is_point_inside_area(
				point,
				node_ref,
				layout_node,
				scale_factor,
			),
			Self::Label(el) => el.is_point_inside_area(
				point,
				node_ref,
				layout_node,
				scale_factor,
